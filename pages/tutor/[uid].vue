...
<template>
	...
	<div class="user-detail-header_mobile" v-else-if="userData && !isLoading">
		...
		<div class="user-contact">
					...
					<div class="user-contact_details">
						<div class="user-contact_review">
							<svg class="icon-md" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
								viewBox="0 0 24 24" fill="#f9ce69" stroke="currentColor" stroke-width="2"
							...
							(<span class="text-small gray-500">{{ userData.info_reviews.total_reviews }} Đánh
								giá</span>)
						</div>
			...
			<div class="action-group mobile-contact-actions">
				<button class="action-btn btn-secondary" @click="openSendMessageModal">
				...
				<button class="action-btn btn-primary" @click="redirectToBooking()">
					<svg class="btn-icon" xmlns="http://www.w3.org/2000/svg" data-preply-ds-component="SvgTokyoUIIcon"
						viewBox="0 0 24 24" aria-hidden="true" focusable="false" fill="white">
						<path fill-rule="evenodd" d="m14 10 1-8L2 14h9l-1 8 12-12z" clip-rule="evenodd"></path>
					</svg>
	...
	<div class="user-detail-page" v-if="userData && !isLoading && userData.uid">
		<div class="container">
			<div class="user-detail-header_desktop">
				<div class="user-detail-header">
					...
					<div class="header-main-block">
						...
						<div class="subject-tags">
							<span class="subject-tag" v-for="item in userData.user_subjects" :key="item.id">{{
					...
					<div class="action-group price-actions">
						...
						<button class="action-btn btn-primary" @click="redirectToBooking()">
							<svg class="btn-icon" xmlns="http://www.w3.org/2000/svg"
								data-preply-ds-component="SvgTokyoUIIcon" viewBox="0 0 24 24" aria-hidden="true"
			...
			<div class="user-detail-main-v2">
				<div class="type-selector">
					<button v-for="tab in tabs" :key="tab.id" :class="['type-button', { active: activeTab === tab.id }]"
		...
		<SendMessage v-if="showSendMessageModal" :user="selectedUser" :isOpen="showSendMessageModal"
			@messageSent="handleMessageSent" @close="showSendMessageModal = false" />
	</div>
</template>
