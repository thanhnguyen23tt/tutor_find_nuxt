...
const { status_classroom: classroomStatus } = useConfig();
const { getParticipantsClass, getStatusClass, formatDateTime, diffDateTimeText, getFirstCharacterOfLastName } = useHelper();
...
const tabs = computed(() => configStore.configuration.classroom_status || {})
...
const processClassroomResponse = (response) => {
	classrooms.value = (response?.data || []).map(item => ({...item}));
...
const socketHandlers = {
	classroomCreated: (e) => {
		console.log('classroom:created', e);
		changePage(currentPage.value);
	},

	userJoined: (e) => {
		console.log('classroom:user_joined', e);
		const classroomId = e.classroomId || e.classroom_id;
		if (classroomId && e.participantCount !== undefined) {
	...
	userLeft: (e) => {
		console.log('classroom:user_left', e);
		const classroomId = e.classroomId || e.classroom_id;
		if (classroomId && e.participantCount !== undefined) {
			updateClassroomData(classroomId, {
				participants_count: e.participantCount
	...
	classroomStarted: (e) => {
		console.log('classroom_started', e);
		updateClassroomData(e.classroom_id, {
			status: e.status,
...
// Helper to update classroom data
const updateClassroomData = (classroomId, updates) => {
...
const initSocketIO = async () => {
	...
	try {
		...
		if (socket && userData.value?.uid) {
			socket.emit('classroom-manager:join');

			// Bind event listeners - legacy events from Laravel
			socket.on('classroom:created', socketHandlers.classroomCreated);
			socket.on('classroom_started', socketHandlers.classroomStarted);
			socket.on('classroom_ended', socketHandlers.classroomEnded);

			// Bind event listeners - Node.js events (same as classroom-detail.vue)
			socket.on('classroom:user_joined', socketHandlers.userJoined);
			socket.on('classroom:user_left', socketHandlers.userLeft);

			console.log('✅ Socket.IO classroom-manager listeners initialized');
		}
	} catch (e) {
...
const cleanupSocketIO = async () => {
	if (!process.client || !socket) return;
	
	try {
		socket.emit('classroom-manager:leave');
		socket.off('classroom:created', socketHandlers.classroomCreated);
		socket.off('classroom_started', socketHandlers.classroomStarted);
		socket.off('classroom_ended', socketHandlers.classroomEnded);
		socket.off('classroom:user_joined', socketHandlers.userJoined);
		socket.off('classroom:user_left', socketHandlers.userLeft);
	} catch (e) {
		console.error('Socket.IO cleanup failed:', e);
	}
};
...
const startClassroom = async (classroom) => {
	loadingStates.value.starting = classroom.id;
	try {
		if (!classroom.can_start) {
			notifyWarning(classroom.time_info_text || 'Chưa đến giờ học');
			return;
		}
		...
		if (res?.success && res?.data) {
			...
			updateClassroomData(classroom.id, res.data);
		...
		notifyError(e?.message || 'Có lỗi xảy ra khi bắt đầu lớp học');
	} finally {
		loadingStates.value.starting = null;
	}
};

const joinClassroom = (classroom) => {
	loadingStates.value.joining = classroom.id;
	...
	try {
		...
		notifyError(e?.message || 'Có lỗi xảy ra khi tham gia lớp học');
	} finally {
		loadingStates.value.joining = null;
	}
};

const retryClassroom = async (classroomId) => {
	loadingStates.value.retrying = classroomId;
	try {
		...
		loadingStates.value.retrying = null;
	}
};

const shareClassroom = async (classroom) => {
	loadingStates.value.sharing = classroom.id;
	try {
		if (!process.client) return;
		...
		loadingStates.value.sharing = null;
	}
};
...
const openClassroomDetail = async (classroom) => {
	...
	try {
		...
		selectedClassroom.value = response?.success ? response.data : classroom;
...
<template>
	...
	<BasePageError v-else-if="initialClassroomsError"
		:message="initialClassroomsError.message || 'Không thể tải danh sách lớp học'" />
	...
	<div class="classroom-manager-page" v-else>
		<div class="container">
			...
			<div class="classroom-manager-toolbar">
				<div class="search-group">
					<base-input v-model="search" placeholder="Tìm kiếm theo chủ đề, mã booking hoặc tên học viên..."
						@keyup.enter="handleSearch"></base-input>
					<button class="btn-primary btn-lg w-max" @click="handleSearch">
						<div>
							<svg xmlns="http://www.w3.org/2000/svg" class="icon-md" width="20" height="20"
			...
			<div class="classroom-card" v-else>
				<div v-for="classroom in filteredClassrooms" :key="classroom.id" class="classroom-card-wrapper"
					@click="openClassroomDetail(classroom)">
					<div class="classroom-card-left">
						...
						<div class="content">
							...
							<div class="classroom-duration">
								<svg class="icon-sm" width="16" height="16" fill="none" stroke="currentColor"
							...
							<div class="classroom-participants" v-if="classroom.status !== 'ended'">
								...
								<span :class="getParticipantsClass(classroom)">
									{{ classroom.participants_count || 0 }}/{{ classroom.max_participants }}
								</span>
							</div>
					...
					<div class="classroom-card-center">
						<div class="classroom-info">
							<div class="info-row">
								<svg class="icon-sm" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
									viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
							...
							<div class="info-row">
								...
								<span><span>Thời gian:</span> {{ formatDateTime(classroom.scheduled_start_time)
									}}</span>
							</div>
							<div v-if="classroom.time_info_text" class="info-row time-status">
								<svg class="icon-sm" width="16" height="16" fill="none" stroke="currentColor"
									stroke-width="2" viewBox="0 0 24 24">
									<circle cx="12" cy="12" r="10" />
									<polyline points="12 6 12 12 16 14" />
								</svg>
								<span>{{ classroom.time_info_text }}</span>
							</div>
					...
					<div class="classroom-card-right">
						<div class="action-group" @click.stop v-if="classroom.status !== 'ended'">
							<template v-if="classroom.status === 'waiting'">
								<button class="action-btn btn-primary" :disabled="!classroom.can_start || loadingStates.starting"
									:class="{ 'btn-loading': loadingStates.starting }"
									@click.stop="startClassroom(classroom)">
									<svg class="btn-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
										fill="none" stroke="white" stroke-width="2">
										<polygon points="5 3 19 12 5 21 5 3"></polygon>
									</svg>
									<span class="btn-text">{{ loadingStates.starting ? 'Loading...' : 'Bắt đầu' }}</span>
								</button>
							</template>
							<template v-else-if="classroom.status === classroomStatus.started">
								<button class="action-btn btn-secondary" :disabled="!classroom.can_start || loadingStates.joining"
									:class="{ 'btn-loading': loadingStates.joining }"
									@click.stop="classroom.status === classroomStatus.started ? joinClassroom(classroom) : startClassroom(classroom)">
									<svg class="btn-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
									...
									<span class="btn-text">{{ loadingStates.joining ? 'Loading...' : 'Tham gia' }}</span>
								</button>
							</template>
							<template v-else-if="classroom.status === classroomStatus.cancelled">
							...
							<template v-else-if="classroom.status === classroomStatus.error">
								<button class="action-btn btn-danger" @click.stop="retryClassroom(classroom.id)"
									:disabled="retryingClassroom === classroom.id"
									:class="{ 'btn-loading': retryingClassroom === classroom.id }">
									<svg class="btn-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
										fill="none" stroke="white" stroke-width="2">
									...
									<span class="btn-text">{{ loadingStates.retrying ? 'Đang thử lại...' : 'Thử lại' }}</span>
								</button>
							</template>
						</div>
						<!-- More Menu -->
							...
							<base-more-menu>
								<template #activator="{ toggle }">
									<div class="more-btn" @click.stop="toggle">
										<svg class="icon-sm" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
											viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
			...
			<base-pagination :meta="dataPaginate" :current-page="currentPage"
				@changePage="changePage"></base-pagination>
		</div>

		<!-- Classroom Detail Modal -->
		<base-modal v-if="showDetailModal" :isOpen="showDetailModal" :header="false" @close="showDetailModal = false"
			size="medium">
			<div v-if="selectedClassroom" class="classroom-detail-content">
				<!-- Header Section -->
				<div class="detail-header">
					<div class="classroom-avatar-large">
						<svg xmlns="http://www.w3.org/2000/svg" class="icon-max" viewBox="0 0 24 24" fill="none"
							stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
							<g style="transform: none; transform-origin: 4px 20px 0px; transform-box: fill-box;">
								<g style="transform: none; transform-origin: 3px 11px 0px; transform-box: fill-box;">
					...
					<div class="header-info">
						...
						<div class="classroom-meta">
							...
							<span class="classroom-code-large">Mã booking: {{ selectedClassroom.booking?.request_code
								}}</span>
						</div>
					</div>
				</div>
						...
						<div class="info-grid">
							...
							<div class="info-item" v-if="selectedClassroom.status !== 'ended'">
								...
								<div class="info-value" :class="getParticipantsClass(selectedClassroom)">
									{{ selectedClassroom.participants_count || 0 }}/{{
										selectedClassroom.max_participants || 2
									}}
								</div>
							</div>
							<div class="info-item">
								...
								<div class="info-value">{{ formatDateTime(selectedClassroom.scheduled_start_time) }}
								</div>
							</div>
					...
					<div class="detail-section">
						...
						<div class="expected-participants">
							<div class="participants-info">
								<div class="participant-card participant-clickable"
									@click="openProfileModal(selectedClassroom.booking?.tutor)">
									<div class="participant-avatar">
										<img v-if="selectedClassroom.booking?.tutor?.avatar"
											:src="selectedClassroom.booking?.tutor?.avatar"
											:alt="selectedClassroom.booking?.tutor?.full_name">
										<svg v-else class="icon-md" xmlns="http://www.w3.org/2000/svg" width="20"
									...
									<div class="participant-details">
										<div class="participant-name">{{ selectedClassroom.booking?.tutor?.full_name }}
										</div>
										<div class="participant-role">Giáo viên</div>
									</div>
								</div>
								<div class="participant-card participant-clickable"
									@click="openProfileModal(selectedClassroom.booking?.user)">
									<div class="participant-avatar">
										<img v-if="selectedClassroom.booking?.user?.avatar"
											:src="selectedClassroom.booking?.user?.avatar"
											:alt="selectedClassroom.booking?.user?.full_name">
										<svg v-else class="icon-md" xmlns="http://www.w3.org/2000/svg" width="20"
											height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
											stroke-width="2">
											<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"></path>
									...
									<div class="participant-details">
										<div class="participant-name">{{ selectedClassroom.booking?.user?.full_name }}
										</div>
						...
						<!-- Actual Participants -->
						<div v-if="selectedClassroom.participants && selectedClassroom.participants.length > 0"
							class="actual-participants">
							<h4 class="subsection-title">Người đã tham gia</h4>
							<div class="participants-info">
								<div v-for="participant in selectedClassroom.participants" :key="participant.user_id"
									class="participant-card actual-participant">
										...
										<div class="participant-times">
											<div class="time-info">
												...
												<span class="time-value">{{
													diffDateTimeText(participant.last_activity_at,
													participant.first_joined_at) }}</span>
											</div>
										</div>
									</div>
								</div>
				...
				<div class="modal-footer">
					...
					<button v-if="selectedClassroom.status === 'waiting'" class="btn-md btn-primary"
						:disabled="!selectedClassroom.can_start" @click="startClassroom(selectedClassroom)">
						<svg class="icon-sm" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
							viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
							<polygon points="5 3 19 12 5 21 5 3"></polygon>
						</svg>
					...
					<button v-else-if="selectedClassroom.status === 'started'" class="btn-md btn-secondary"
						:disabled="!selectedClassroom.can_start" @click="joinClassroom(selectedClassroom)">
						<svg class="icon-sm" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
							viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
							<path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"></path>
							<polyline points="10,17 15,12 10,7"></polyline>
		...
		<!-- Profile Modal -->
		<base-modal v-if="showProfileModal" :isOpen="showProfileModal" @close="showProfileModal = false"
			title="Thông tin cơ bản" size="medium">
			<div v-if="selectedProfileUser" class="profile-modal-content">
				<div class="profile-header">
					<div class="profile-avatar-large">
						<img v-if="selectedProfileUser.avatar" :src="selectedProfileUser.avatar"
							:alt="selectedProfileUser.full_name">
						<span v-else class="avatar-text-large">{{
							getFirstCharacterOfLastName(selectedProfileUser.full_name)
							}}</span>
					</div>
					<div class="profile-info">
						...
						<div v-if="selectedProfileUser.info_reviews?.average_rating" class="profile-rating">
							...
							<span class="rating-count">({{ selectedProfileUser.info_reviews.total_reviews }} đánh
								giá)</span>
						</div>
					</div>
				</div>
				...
				<div class="profile-details">
					...
					<div class="detail-section"
						v-if="selectedProfileUser.role === 'tutor' && selectedProfileUser.user_subjects?.length">
						<h3 class="section-title">Môn học giảng dạy</h3>
						<div class="subjects-grid">
							<div v-for="subject in selectedProfileUser.user_subjects" :key="subject.id"
								class="subject-chip">
								<svg class="icon-sm" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
