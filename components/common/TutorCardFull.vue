	...
	<div class="tutor-card" @click="navigateToTutor(tutor.uid)">
		<div class="tutor-card-wrapper">
			<div class="tutor-header-wrapper">
				<img :src="showImage(tutor.avatar)" alt="" class="tutor-image">

				<div class="tutor-header_detail">
					<div class="tutor-name">
						<span>{{ tutor.full_name }}</span>
						<svg aria-label="Đã xác minh" class="icon-md" fill="rgb(0, 149, 246)" height="12" role="img"
				...
				<button class="btn-like" @click.stop="toggleSave(tutor)" title="Yêu thích" v-if="!tutor.is_saved">
					<svg xmlns="http://www.w3.org/2000/svg" class="icon-md" viewBox="0 0 24 24" fill="none"
			...
			<div class="tutor-info">
				<div class="subject-list">
					<div class="subject-row">
						<span v-for="subject in tutor.user_subjects" :key="subject.id" class="subject-tag">
				...
				<div class="info-list">
					...
					<div class="info-item">
						...
						<div class="info-wrapper">
							<div class="info-title">{{tutor.languages_preview }}</div>
...
<span>{{ formatCurrency(tutor.average_price) }}/giờ</span>
					<div v-if="showPriceDetail === tutor.id" class="price-detail-popup">
						<div class="popup-title">Học phí theo môn học và cấp độ:</div>
						<div v-for="(subject, sidx) in tutor.user_subjects" :key="subject.id"
							class="price-detail-subject">
	...
	<SendMessage v-if="showSendMessageModal" :user="tutor" :isOpen="showSendMessageModal"
...
const { formatCurrency, showImage } = useHelper();
