                ...
                <div class="card-content">
                    <div class="confirm-main-row">
						...
						<div class="confirm-info-grid">
                            ...
                            <div class="confirm-value">Tính theo giờ</div>
                            <template>
                        ...
                        <div class="payment-methods-grid">
                            <div v-for="method in paymentMethods" :key="method.id" class="payment-method-item" :class="{ active: selectedPaymentMethod?.code === method.code }" @click="selectPaymentMethod(method)">
                                <div class="payment-method-icon">
                                    <img :src="showImage(method.icon)" :alt="method.name" />
                                </div>
...
const { formatDate, formatDuration, formatCurrency, handleTimeSlot, showImage } = useHelper()
...
const selectedLevel = computed(() =>
    selectedSubject.value?.user_subject_levels?.find(
        (lvl) => lvl.id === bookingData.value.level.id
    )
)
...
const validateBookingData = (data) => {
    ...
    if (isToday && data.time?.start) {
        // Validation without timeOptions lookup
        const slotTime = moment(data.time.start, 'HH:mm').format('HH:mm')
...
const submitVnpayPayment = async () => {
    try {
        ...
        const payload = {
            ...
            subject_level_id: parseInt(bookingData.value.level.id, 10),
            date: bookingData.value.date,
            start_time: bookingData.value.time.start,
            tutor_session_id: selectedTutorSessionId.value,
            weekly_time_slot_id: bookingData.value.weekly_time_slot_id,
		...
		return navigateTo(res?.paymentUrl, { external: true })
        } else {
